...
def main():
    ...
    #onnx.save(onnx.shape_inference.infer_shapes(optimized_onnx_model), "model_test.onnx")
    #return
...
def extract_model(model: onnx.ModelProto, input_name):
    ...
    output_to_shape = (
        (
            "/model/model.105/m.0/Conv_output_0",
            (1, 255, int(INPUT_SHAPE[0] / 8), int(INPUT_SHAPE[1] / 8)),
        ),
        (
            "/model/model.105/m.1/Conv_output_0",
            (1, 255, int(INPUT_SHAPE[0] / 16), int(INPUT_SHAPE[1] / 16)),
        ),
        (
            "/model/model.105/m.2/Conv_output_0",
...
def build_argument_parser():
    ...
    parser.add_argument(
        "--onnx_path", type=str, default="yolov7.onnx", help="Path to onnx file"
    )
    parser.add_argument(
        "--output_path",
        type=str,
